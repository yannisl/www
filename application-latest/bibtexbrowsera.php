...
@define('BIBTEXBROWSER_DEBUG',true);
    ...
    if ($this->displayer!='') {
      ...
      $this->clearQuery();
       ...
       if ( headers_sent() == false ) { /* to avoid sending an unnecessary frameset */ 
         // everything now is in the array $_GET  
         //echo $_SERVER['SCRIPT_NAME'];
         //foreach($this->entries as $bibentry) { echo $bibentry->getText()."\n"; }
         $selectedEntries = array_values($_GET);
         //echo_array($_GET);//break;
...
$class = BIBTEXBROWSER_MAIN;// extension point creates new dispatcher
$main = new $class();
 //echo_array($_GET);break;
$main->main();
?>